...
from data.capdata_loader import capdata_train, capdata_eval
...
def create_dataset(dataset, config, min_scale=0.5):
    ...
    pretrain_transform = transforms.Compose([
            transforms.Resize(int(config['image_size'] * 1.1), interpolation=Image.BICUBIC),
    ...
    if dataset=='pretrain':
        dataset = pretrain_dataset(config['train_file'], config['laion_path'], pretrain_transform)              
        return dataset  
    
    elif dataset=='caption_peppa':   
        train_dataset = capdata_train(pretrain_transform, config['image_root'], config['ann_root'], prompt=config['prompt'])
        val_dataset = capdata_eval(transform_test, config['image_root'], config['ann_root'], 'val')
        test_dataset = capdata_eval(transform_test, config['image_root'], config['ann_root'], 'test')   
        return train_dataset, val_dataset, test_dataset
    
    elif dataset=='caption_coco':   
        train_dataset = coco_karpathy_train(pretrain_transform, config['image_root'], config['ann_root'], prompt=config['prompt'])
    ...
    elif dataset=='retrieval_peppa':          
        train_dataset = capdata_train(transform_train, config['image_root'], config['ann_root'])
        val_dataset = capdata_eval(transform_test, config['image_root'], config['ann_root'], 'val') 
        test_dataset = capdata_eval(transform_test, config['image_root'], config['ann_root'], 'test')          
        return train_dataset, val_dataset, test_dataset   
